    ...
    <div className="p-5 m:p-8 bg-white">
      ...
      <div className="flex gap-x-7 mt-5 flex-col m:flex-row gap-y-1 m:gap-y-0">
        ...
        <div className="w-[370px] flex flex-col">
          <textarea
            {...register("biography", { max: 150 })}
            className="border-[1px] rounded border-gray outline-0 max-h-[70px] py-1 w-full"
          ></textarea>
          <p
            className={`${
              watch().biography?.length > 150 ? "text-red-600" : "text-gray"
            } text-xs mt-4`}
          >
            {watch().biography?.length || 0}/150
          </p>
        </div>
      </div>
      ...
      <div className="flex justify-center gap-x-[5rem] md:ml-[10rem] mt-10">
        <Button
          onClick={() => onSubmit()}
          modifiers="py-1 px-3 bg-blue text-white rounded text-sm font-semibold"
        >
          Trimite
        </Button>
